    ...
    BR: BlockReranker<'a, Parser::Block<'a>> + Send + 'static,
{
    ...
    async fn llm_request_for_blocks(
        ...
        if suspicious_vars.is_none() {
            // dive — LLM can't decide, use full dataflow
            let early_stop = if self.early_stop {
                EarlyStop::Block
            } else {
                EarlyStop::None
            };
            res.map(|(next_scope, next_vars, _)| (next_scope, next_vars, early_stop))
        } else {
            // not dive — LLM selected signals, but still use full dataflow
            // to ensure data path is not filtered out (paper Algorithm 1 traces ALL deps).
            // LLM's suspicious marking is handled separately in llm_voting_for_blocks.
            res
        }
    }
