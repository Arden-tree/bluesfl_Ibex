...
pub fn parse_name(raw_name: &[u8], length: u32) -> Result<(String, Option<VarIndex>, Vec<String>)> {
    ...
    if c < 30 {
        eprintln!("parse_name: raw={:?} len={}", String::from_utf8_lossy(raw_name), length);
    }
    ...
    while name.last().cloned() == Some(b']') {
        let index_start = match find_last(name, b'[') {
            Some(s) => s,
            None => {
                // Chisel-generated Verilog may have malformed bracket syntax.
                // Reset name to the original (before extract_suffix_index stripped anything)
                // and skip multi-dimensional parsing entirely.
                name = raw_name;
                indices.clear();
                break;
            }
