...
pub(crate) fn read_hierarchy_entry(
    ...
    let entry = match entry_tpe {
        ...
        entry_tpe if entry_tpe < HIERARCHY_TPE_VCD_ATTRIBUTE_BEGIN => {
            // Variable entry (0-251 are all variable types, even if we don't recognize all of them)
            let tpe = FstVarType::try_from_primitive(entry_tpe)
                .unwrap_or(FstVarType::Wire); // fallback for unknown extended types
            let direction = FstVarDirection::try_from_primitive(read_u8(input)?)
                .unwrap_or(FstVarDirection::Implicit); // fallback for unknown directions
            let name = read_c_str(input, HIERARCHY_NAME_MAX_SIZE)?;
        ...
        HIERARCHY_TPE_VCD_ATTRIBUTE_BEGIN => {
            ...
            match tpe {
                ...
                AttributeType::Array => {
                    // FST array attribute: read and skip array dimensions
                    // Format: num_dims (variant u64), then for each dim: hi (variant u64), lo (variant u64)
                    let (num_dims, _) = read_variant_u64(input)?;
                    for _ in 0..num_dims {
                        let _ = read_variant_u64(input)?; // hi
                        let _ = read_variant_u64(input)?; // lo
                    }
                    FstHierarchyEntry::AttributeEnd
                }
                AttributeType::Enum => {
                    // FST enum attribute: read and skip
                    // Format: name (null-terminated), then enum table data
                    let _name = read_c_str(input, HIERARCHY_ATTRIBUTE_MAX_SIZE)?;
                    let (mapping_len, _) = read_variant_u64(input)?;
                    for _ in 0..mapping_len {
                        let _ = read_c_str(input, HIERARCHY_ATTRIBUTE_MAX_SIZE)?;
                        let _ = read_c_str(input, HIERARCHY_ATTRIBUTE_MAX_SIZE)?;
                    }
                    FstHierarchyEntry::AttributeEnd
                }
                AttributeType::Pack => {
                    // FST pack attribute: read and skip
                    // Format: same as Misc - name + arg
                    let _name = read_c_str(input, HIERARCHY_ATTRIBUTE_MAX_SIZE)?;
